...
import LoaderCompletedlist from "../Components/LoaderCompletedlist";
...
const OrderRecived = () => {
  ...
  const Done = Orders?.filter(order => order.Completed) || [];
  ...
  if (status === 'loading') {
    return <div><LoaderCompletedlist/></div>;
  }
  ...
  return (
    ...
    <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        <h2 className="text-3xl font-bold text-center text-gray-900 mb-8">Order History</h2>
        ...
        <Menu.Items className="z-10 mt-2 w-full bg-white border border-gray-300 rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none max-h-60 overflow-y-auto">
          {order.order_data.products.map((product) => (
            ...
            <div className="mt-4 flex space-x-2">
              ...
              <button
                onClick={() => handleDelete(order.id, order.order_data.fullName)}
